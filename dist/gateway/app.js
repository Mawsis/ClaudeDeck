...
import { requireAnyKey, requireWorkspace } from "./auth.js";
import { classifyBash } from "./bash-classifier.js";
import { registerMintRoutes } from "./mint-routes.js";
import { registerPermissionRoutes } from "./permission-routes.js";
import { DEFAULT_QUESTION_TIMEOUT_MS, registerQuestionRoutes } from "./question-routes.js";
import { createPushRegistry } from "./push-registry.js";
import { loadBrandAssets, loadDeckReducerJs, loadPwaHtml, loadServiceWorkerJs } from "./static.js";
import { clampDetail, extractToolDetail } from "./tool-detail.js";
...
function parsePushSubscription(body) {
    if (typeof body !== 'object' || body === null)
        return undefined;
    const record = body;
    if (typeof record.endpoint !== 'string' || !isHttpsUrl(record.endpoint))
        return undefined;
...
function parseHookPayload(body) {
    ...
    const type = HOOK_EVENT_TYPES[record.hook_event_name];
    if (type === undefined)
        return undefined;
    return { type, ...common };
}
export function createApp(config) {
    ...
    app.get('/deck-reducer.js', (c) => c.body(deckReducerJs, 200, {
    ...
    app.get('/sw.js', (c) => c.body(serviceWorkerJs, 200, {
        'Content-Type': 'text/javascript; charset=utf-8',
        'Cache-Control': NO_CACHE,
    }));
    app.post('/api/events', requireWorkspace('hook', workspaceStore), async (c) => {
        ...
        if (payload === undefined) {
            return c.json({
                error: 'expected a Stop, UserPromptSubmit, PostToolUse, or Handshake hook payload with session_id and cwd',
            }, 400);
        }
        ...
        const event = payload.type === 'tool'
            ? eventLog.publish({
                ...
                ...(payload.tool_name === 'Bash'
                    ...
                    : { category: 'edit', risk: 'routine' }),
            })
            : eventLog.publish({ type: payload.type, ...base });
        return c.json({ id: event.id }, 202);
    });
    app.get('/api/deck-config', requireWorkspace('deck', workspaceStore), (c) => c.json({
        alertThresholdMs,
    ...
    app.post('/api/push/subscriptions', requireWorkspace('deck', workspaceStore), async (c) => {
        ...
        if (subscription === undefined) {
            return c.json({ error: 'expected a Web Push subscription with endpoint and keys.p256dh/keys.auth' }, 400);
        }
    ...
    app.post('/api/pause', requireWorkspace('deck', workspaceStore), async (c) => {
        let requested;
        try {
            requested = (await c.req.json()).paused;
        }
        catch {
            requested = undefined;
        }
        const { eventLog, pauseState } = runtimeFor(c.get('workspaceId'));
        const paused = typeof requested === 'boolean' ? pauseState.set(requested) : pauseState.toggle();
    ...
    app.get('/api/stream', requireWorkspace('deck', workspaceStore), (c) => {
        ...
        return streamSSE(c, async (stream) => {
            ...
            const writeEvent = (event) => void stream.writeSSE({
                id: String(event.id),
                event: event.type,

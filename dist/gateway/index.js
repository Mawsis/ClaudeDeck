...
import { loadConfigFromEnv } from "./env-config.js";
import { createWorkspaceStore } from "./workspace-store.js";
const config = loadConfigFromEnv(process.env);
let alerts = { thresholdMs: config.alertThresholdMs };
...
const workspaceStore = createWorkspaceStore(config.dbPath !== undefined ? { db: new DatabaseSync(config.dbPath) } : {});
...
serve({ fetch: app.fetch, port: config.port }, (info) => {
    console.log(`slopdeck gateway listening on :${info.port}` +

...
function presentedToken(c, scope) {
 ...
 */
export function requireScope(scope, tokens) {
    ...
    return async (c, next) => {
        ...
        if (safeEquals(presented, expected))
            return next();
        if (safeEquals(presented, other))
            return c.json({ error: 'wrong token scope' }, 403);
 ...
 */
export function requireWorkspace(scope, store) {
    return async (c, next) => {
        const presented = presentedToken(c, scope);
        if (presented === undefined)
            return c.json({ error: 'missing token' }, 401);
 ...
 */
export function requireAnyKey(store) {
    return async (c, next) => {
        const header = c.req.header('Authorization');

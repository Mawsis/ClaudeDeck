...
export function reduceDeck(state, event) {
    ...
    /** @type {SessionState} */
    const session = event.type === 'prompt'
        ? {
            status: 'running',
            title: event.title,
        ...
        : {
            ...
            elapsedMs: previous?.status === 'running'
...
export function completionAlert(state, event, options = {}) {
    ...
    if (event.type !== 'stop')
        return null;
    if (now !== undefined && now - event.at > MAX_ALERT_AGE_MS)
        return null;
    const previous = state.sessions[event.sessionId];
...
export function reducePrompts(prompts, event) {
    ...
    if (prompts.some((prompt) => prompt.promptId === event.promptId))
        return prompts;
    const base = {
    ...
    if (event.type === 'question') {
        return [
            ...prompts,
            {
                ...
                ...base,
                questions: (Array.isArray(event.questions) ? event.questions : []).map(normalizeQuestionSpec),
            },
        ];
    }
    return [
...
export function reduceQuestionDraft(questions, draft, action) {
    ...
    if (action.type === 'tap') {
        ...
        return {
            ...draft,
            selected: toggledOff.length < draft.selected.length ? toggledOff : [...draft.selected, action.choice],
        };
    }
    // Confirm is meaningful only on a multiSelect step with something chosen —
    // an empty set is not an answer.
    if (!current.multiSelect || draft.selected.length === 0)
        return draft;
...
export function handshakeWave(event, receiptNow) {
    if (event.type !== 'handshake')
        return null;
    if (receiptNow - event.at > MAX_ALERT_AGE_MS)
        return null;
...
function clampBubbleLine(line) {
    if (Array.from(line).length <= BUBBLE_LINE_MAX)
        return line;
...
export function bubbleLine(bubble) {
    ...
    if (labeled === undefined)
        return clampBubbleLine(bubble.detail);
...
export function deckTransition(before, after) {
    if (!before.promptPending && after.promptPending)
        return 'crt';

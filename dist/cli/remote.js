 ...
 */
export async function setInterception(deps, on) {
    ...
    if (!result.ok) {
        ...
        io.say(result.error === 'unauthorized'
            ? 'deck token rejected by the gateway — interception state unchanged'
            : `could not turn interception ${on ? 'on' : 'off'}: gateway ${result.error === 'unreachable' ? `unreachable (${result.detail})` : `error (http ${result.status})`}`);
        return { ok: false };
    }
...
export async function status(deps) {
    ...
    let failed = false;
    const good = (line) => io.say(`  ok    ${line}`);
    const bad = (line) => {
        failed = true;
        io.say(`  FAIL  ${line}`);
    };
    const skip = (line) => io.say(`  skip  ${line}`);
    const config = await readCliConfig(files, paths.configFile);
    if (config.ok)
        good(`config present at ${paths.configFile} (gateway ${config.config.gatewayUrl})`);
    else
        bad(config.error);
    const settings = await files.read(paths.claudeSettings);
    if (settings !== null && hasSlopdeckHooks(settings))
    ...
    if (hookToken === undefined || hookToken === '') {
        ...
        if (verified.ok)
            good('hook token accepted by the gateway');
        else if (verified.error === 'unauthorized')
            bad('hook token rejected by the gateway');
        else
            bad(`hook token check failed: gateway error (${verified.error})`);
    }
    ...
    if (!health.ok) {
        ...
        if (paused.ok)
            good(`deck key resolves its workspace — interception ${paused.value ? 'off (paused)' : 'on'}`);
        else if (paused.error === 'unauthorized')
            bad('deck key rejected by the gateway — the workspace it names no longer exists');
        else
            bad(`deck key check failed: gateway error (${paused.error})`);
    }
    return { ok: !failed };
}
